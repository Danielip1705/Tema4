...
public class Ej9 {
	...
	public static boolean esJavalandia(String frase) {
		...
		//Declaramos con if que si frase empieza con Javalín, javalón o termina con javalén, len, len(quitando el espacio con .trim)
		if (frase.startsWith("Javalín, javalón".trim()) | frase.endsWith("".trim() + "javalén, len, len")) {
	...
	public static String traducido(String frase) {
		...
		if (frase.startsWith("Javalín, javalón")) {
			...
			traducido = frase.substring("Javalín, javalón\t".length() + 1).trim();
			
			// y si no
		} else {
			
			//Que traducido se inicialice a frase quitando Javalín, javalén, len, len
			traducido = frase.substring(0, frase.length() - "\tjavalén, len, len".length() - 1).trim();
		}
